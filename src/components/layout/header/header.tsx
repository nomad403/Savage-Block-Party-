...
import { menuEvents } from "@/lib/events/app-events";
...
export default function Header() {
    ...
    const { isHome, isAgenda, isFamily, isPresse, isShop } = usePageContext();
    
    // Utiliser les couleurs dynamiques globales (source unique de vérité)
    const { colors } = useGlobalDynamicColors();
    const { hoveredMenuItem } = useMenuHover();
     ...
     * ========================================
     * 
     * SOURCE UNIQUE DE VÉRITÉ : useGlobalDynamicColors() hook
     * 
     * Toutes les couleurs sont calculées dans useGlobalDynamicColors() qui prend en compte :
     * - La page actuelle
     * - Le hover du menu (via useMenuHover)
     * - Le hover/sélection des items shop (via useShopItemState)
     * 
     * Le header utilise uniquement les couleurs exposées par useGlobalDynamicColors :
     * - colors.logoColor : couleur du logo (déjà calculée avec toute la logique)
     * - colors.menuColor : couleur du texte du menu (déjà calculée avec toute la logique)
     * 
     * ARCHITECTURE :
     * - useGlobalDynamicColors() : calcule toutes les couleurs (source unique)
     * - header.tsx : consomme les couleurs (composant passif)
     * - header.tsx émet événements menuItemHover pour synchroniser useMenuHover()
     ...
     */
    // Utiliser les couleurs centralisées de useGlobalDynamicColors
    const logoColor = colors.logoColor;
    const menuTextColor = colors.menuColor;
    const cartIconColor = colors.logoColor; // Même logique que le logo
    ...
    // La couleur du burger menu est maintenant gérée par les classes CSS conditionnelles
    // appliquées par DynamicColorProvider (shop-item-selected, shop-item-hovered, menu-hovered)
                    ...
                    <div className="hidden md:flex items-center gap-3 shrink-0">
                        ...
                        <Link 
                            ...
                            style={{ 
                                ...
                                height: '36px',
                                color: cartIconColor, // Noir au hover du menu, sinon couleur primaire de la page
                                transition: 'color 0.3s ease'
                            }}
                            title="Shop"
                            onMouseEnter={() => handleMenuItemHover("/shop")}
                            onMouseLeave={() => handleMenuItemHover(null)}
                        >
                            <svg 
					...
					>
						<div className="h-full w-full flex">
								<nav className="ml-auto h-full w-full flex flex-col justify-center items-end gap-0 pr-10 sm:pr-14 relative z-[41]">
									{(() => {
										/**
										 * Composant interne pour gérer la couleur des boutons menu
										 * Utilise maintenant des styles inline au lieu de setProperty avec !important
										 */
										...
										return menuItems.map((item) => {
											...
											// - Exception page shop : si un item est sélectionné/hover → tous les boutons en noir
											let buttonColor: string;
											
											// Exception page shop : si un item est sélectionné ou hover, tous les boutons sont noirs
											if (isShop && isMobile && (isShopItemSelected || isShopItemHovered)) {
												buttonColor = "#000000";
												...
												buttonColor = mobileMenuBgColor !== "transparent" ? mobileMenuBgColor : colors.primary;
											} else {
												// Aucun hover : couleur primaire de la page actuelle
