...
export default function StoryOverlay({ children, className = "" }: StoryOverlayProps) {
  return (
    <motion.div
      ...
      className={`absolute inset-0 flex items-center justify-center z-10 thermal-text-overlay ${className}`}
    >
      {children}
    </motion.div>
  );
...
function LineByLineText({ text, className }: { text: string; className: string }) {
  ...
  return (
    <div ref={containerRef} className="w-full relative">
      {/* Texte au-dessus */}
      <span ref={textRef} className={`${className} text-on-thermal`} style={{ position: 'relative', zIndex: 1 }}>{text}</span>
  ...
  return (
    <div className={`max-w-2xl transition-opacity duration-300 ${isMenuOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'} ${className}`}>
      {title && (
        <LineByLineText 
          text={title} 
          className="font-title text-5xl md:text-6xl text-red-600 mb-6 leading-tight text-justify break-words"
        />
      )}
        ...
        <div className="space-y-4">
          {descriptions.map((desc, idx) => (
            <LineByLineText 
              key={idx}
              text={desc} 
              className="font-text text-lg md:text-xl text-red-600 leading-[1.12] tracking-tight text-justify"
            />
          ))}
        </div>
      )}
...
export function AccrocheText({ text }: { text: string }) {
  ...
  return (
    <div className={`text-center transition-opacity duration-300 ${isMenuOpen ? 'opacity-0 pointer-events-none' : 'opacity-100'}`}>
      <LineByLineText 
        text={text}
        className="font-title text-4xl md:text-6xl text-red-600 leading-tight"
      />
    </div>
  );
}
